...
namespace move_base {
  ...
  */
  ...
  //注意，虽然这里的topic的名字都是goal，但是因为msg的类型不同，应该并不是同一个topic
  ...
  //！！！
  //进行了goal的转换和一些合法性检验;  在这个executeCb函数中对planner_goal_进行了三次更新
  //输入是目标位姿
  ...
  void MoveBase::executeCb(const move_base_msgs::MoveBaseGoalConstPtr& move_base_goal)
  {
    ...
    //开启planner线程
    boost::unique_lock<boost::recursive_mutex> lock(planner_mutex_);
    planner_goal_ = goal;//用MoveBase::executeCb的函数参数move_base_goal更新了一下planner_goal
    runPlanner_ = true;
    planner_cond_.notify_one();
    lock.unlock();
